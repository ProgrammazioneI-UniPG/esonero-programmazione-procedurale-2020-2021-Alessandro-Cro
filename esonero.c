...
char m[129]; //dichiarazione variabili
char k[129];
char c[129];
int a;
...
int main()
{
  ...
  printf("I caratteri in eccesso verranno scartati.\n\n");
...
//se la lunghezza di k è maggiore o uguale alla lunghezza di m, si avvia la cifratura
    if (strlen(k) >= strlen(m)) {
      for ( int i = 0; i < strlen(m) ; i++) {
         ...
         printf("La stringa di testo cifrata è: \n\n %s \n\n", c);
         printf("Lunghezza della stringa cifrata: %lu caratteri\n", strlen(c)-1);
...
if (a == 2) {
  ...
  for (int i = 0 ; i <= strlen(m)-1; ++i) {
    k[i] = (rand() %128); //la chiave viene scritta utilizzando i caratteri della tabella ASCII
